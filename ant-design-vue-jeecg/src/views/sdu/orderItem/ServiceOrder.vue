      ...
      >
        <template slot="title" slot-scope="currentPageData">
          <div style="height: 20px">
            <h3 STYLE="font-weight: bold">企业创新解决方案订单</h3>
          </div>
        </template>
        <template slot="action" slot-scope="text, record">
          <a @click="handleInfo(record)">查看订单</a>
        </template>
        ...
        <template slot="action_confirm" slot-scope="text, record">
          <a-popconfirm
            title="确定要接受这个订单吗?"
            ok-text="确定"
            cancel-text="取消"
...
const columns_1 = [
  ...
  },
  {
    ...
    align: 'center',//设置列内容的对齐方式
    scopedSlots: { customRender: 'action_confirm' },
...
export default {
  ...
  methods: {
    ...
    async getOrderData() {
      ...
      await this.$axios.get('sdu/enterpriseOrder/queryOrderByServiceUser', {
        params: {
          suserName:store.getters.userInfo.username
        }
      }).then(res => {
    ...
    handleOk(e){
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          ...
          this.$axios.post('sdu/enterpriseOrder/deleteByOrderId', this.form).then(res => {
            this.visible = false;
    ...
    async deleteInfo(record){
      ...
      await  this.$axios.post('sdu/enterpriseOrder/deleteByOrderId',record).then(res=>{
        console.log("删除是成功")
        this.getOrderData()
      })
    },
  ...
  //    订单确认状态
    ...
    handleConfirm(record){
      if(record.confirmState==1){
        record.confirmState=2;
        this.getOrderDate(record)
        this.sendServiceNotice(record)

        // console.log(record)
        this.$axios.post('sdu/enterpriseOrder/updateByOrderId',record).then(res=>{
          console.log("订单已被确认")
          this.$message.info("订单已确认")
          this.myConfirm="订单确认成功"
          // this.sendServiceNotice(record)
        })
      }else{
        this.$message.info("订单已被确认，请勿重复操作")

      }
    },

    //  获取订单确认的状态
    getConfirmState(record){
  ...
  //  发送  订单 确认成功的消息
    ...
    sendServiceNotice(record){

      this.notice.name=record.solutionName;
      this.notice.user=record.suserName;
      this.notice.demanduser=record.duserName;
      this.notice.demandname=record.demandName;
      this.notice.date=record.orderSuccess;

      //  orderstate 为 3  表示订单已经被确认
      this.notice.orderstate=3
      if(record.demandid===''){
