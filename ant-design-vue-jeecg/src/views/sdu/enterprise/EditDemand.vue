<template>
  <a-card  style="background-color: white"  >
    <div style="margin-bottom: 3px" slot="title">
      <div style="margin-left: 200px">创新方法推广平台--需求编辑  (操作可见最下方注意事项)</div>
    </div>
    <a-form-model
      ref="ruleForm"
      :model="form"
    ...
    >
      ...
      <a-form-model-item label="企业需求描述" prop="description">
        <J-editor v-model="form.description" class="text" style="padding-bottom: 15px"></J-editor>
...
<!--          <button @click="fileClick()">导入mindfall工具创意方案</button>-->
<!--        </label>-->
<!--        <pre>{{ fileContent }}</pre>-->
      </a-form-model-item>
      ...
      <a-form-model-item :wrapper-col="{ span: 14, offset: 4 }">
        <a-button type="primary" @click="onSubmit()" class="button1">
          需求提交
        </a-button>
      </a-form-model-item>
    </a-form-model>
    <a-card title="注意事项" style="width:80%;margin-left: 200px;margin-top: 60px">
    ...
    async onSubmit() {
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          ...
          await this.$axios.post('sdu/creative/add', this.form).then(res=>{
            console.log(this.form)
