...
<!--    </div>-->
    ...
    <a-table
      ...
      <template slot="demandname" slot-scope="text, record">
       <div>  {{getText(text,record)}}</div>
      </template>
      <template slot="messagetype" slot-scope="text, record">
        <div>  {{getMessageType(text,record)}}</div>
      </template>
...
export default {
      ...
      columns: [
        {
          ...
          //   if (text == '2'||record.orderstate == '3') {
          //     return "需求订单消息";
          //   } else if (text == '2'||record.orderstate == '1' ) {
          //     return "需求订单消息";
          //   }
          //   else if (text == "1") {
          //     return "方案消息";
  ...
  methods: {
    ...
    //  获取具体的方案内容
    async getMessage(){
      await this.$axios.get('sdu/messageNotice/messagePostByUser',{
        params:{
          demanduser:this.$cookies.get('innovation_promotion_username')
        }
      }).then(res=>{
    ...
    showAnnouncement(record){
      ...
      record.readstate=0;
      // console.log(record)
      // console.log("fengexixnai")
      this.updateState(record)

    },
    //  获取这个方案或者订单的数据
    async getAnnouncement(record){
      // console.log(record.name)
      if(record.messagetype==1){
        await this.$axios.get('sdu/solutiondemand/findSolutionByName',{
          params:{
            name:record.name,
          }
        }).then(
          res=>{
            ...
            this.testSource.status=2
            this.$axios.put('sdu/solutiondemand/editById',this.testSource)
    ...
    async updateState(record){
      // console.log(this.testSource)
      await this.$axios.post('sdu/messageNotice/setReadState',record
      ).then(res=>{
        console.log("发送成功")
      }).catch((err)=>{
        console.log(err)
    ...
    async deleteState(record){
      record.readstate=2;
      await this.$axios.post('sdu/messageNotice/setReadState',record).then(res=>{
        console.log("删除成功")
        this.getMessage()
      }).catch((err)=>{
        console.log(err)
    ...
    getText(text,record){
     if(record.demandname==='')
        return "暂无企业需求信息"
      else return text

    },
    getMessageType(text,record){
      if (text == '2'&&record.orderstate == '3') {
            return "交易成功消息";
          } else if (text == '2'&&record.orderstate == '1' ) {
            return "需求订单消息";
    ...
    //         return "已读";
    //       } else if(record.messagetype=='2'||record.messagetype=='3'||text=='1'){
    //         return "订单";
    //       }
    //
    //
