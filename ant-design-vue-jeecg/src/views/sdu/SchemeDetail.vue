...
<!--                   @cancel="cancel"-->
<!--                 >-->
<!--                    <a-button v-if="myRole" type="primary"   class="buttonSet">方案采用</a-button>-->
<!--                 </a-popconfirm>-->
<!--&lt;!&ndash;                 <a-button v-if="myRole" type="primary"   @click="submitSolution" class="buttonSet">方案采用</a-button>&ndash;&gt;-->
<!--               </span>-->
<!--            </a-col>-->
          </a-row>
        </a-form>
...
<!--        {{this.scheme2.solution}}-->
    ...
    >
      <a-textarea placeholder="输入留言内容,点击确定发送给对方" :rows="4" v-model="ModalText"/>
      <a-alert v-if="alertVisible" message="请输入聊天内容,点击确定发送" banner />
    </a-modal>
<!--    <a-button @click="accept">感兴趣</a-button>-->
    <CommentMessage @handleReply="handleReply" :commentList="comments" v-if="hasReset"/>
    <CommentEdit @reload="reload" :parentMsgId="replyMsgId" :replyMsgUsername="replyMsgUsername" :scheme="this.scheme2"/>
  </div>
...
export default {
  ...
  watch:{
    ...
    //     console.log(to.path)
    //     console.log(from)
    //     if(from.path === '/sdu/schemedetail' & to.path === '/sdu/schemedetail'){
    //       this.getNewSolution()
    //     }
    //   }
    // }
