...
//                json.put("avatar", item.getSysUser().getAvatar());
    ...
    public Result<JSONObject> userEmailRegister(@RequestBody JSONObject jsonObject, SysUser user) {
        Result<JSONObject> result = new Result<JSONObject>();
        String email = jsonObject.getString("email");
        String smscode = jsonObject.getString("smscode");
...
//        }
        ...
        try {
            ...
            if(rolecode.equals("1")){
                sysUserService.addUserWithRole(user,"1473923867157839873");
            }else if(rolecode.equals("2")){
                sysUserService.addUserWithRole(user,"1476359056458838018");
            }
            //sysUserService.addUserWithRole(user,"ee8626f80f7c2619917b6236f3a7f02b");//默认临时角色 test
            result.success("注册成功");
        } catch (Exception e) {
...
//	}
    ...
    @GetMapping("/queryPhoneByUsername")
    public String queryPhoneByUsername(String username){
        String res=this.sysUserService.queryPhoneByUsername(username);
        return res;
    }
}
